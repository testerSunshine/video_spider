...
侏罗纪世界"""
    ...
    def getComment(self):
        ...
        while self.redisConn.llen("movice"):
            ...
            print(movie)
            # movie = {'cat': '动作,科幻', 'dir': '迈克尔·贝', 'dur': 154, 'enm': 'Transformers: Dark of the Moon', 'fra': '美国', 'frt': '2011-06-29', 'globalReleased': True, 'id': 47, 'img': 'http://p1.meituan.net/w.h/movie/979866996ad5f872458706eefae8592d320405.jpg', 'movieType': 0, 'nm': '变形金刚3', 'onlinePlay': False, 'pubDesc': '2011-07-21大陆上映', 'rt': '2011-07-21', 'sc': 8.7, 'show': '', 'showst': 2, 'star': '希亚·拉博夫,乔什·杜哈明,约翰·特托罗', 'type': 0, 'ver': '2D/3D/IMAX 3D', 'vodPlay': False, 'wish': 539, 'wishst': 0, 'version': 'v3d imax'}
            offset = movie.get("offset", 0)
            start_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 获取当前时间，从当前时间向前获取
            # end_time = movie.get("rt", "") + "00:00:00"
            while True:
                try:
                    ...
                    elif getCommnetRsp.get("total", ""):  # 如果不返回数据，就代表评论爬到底
                        print("爬取完成")
                        break
